...
docker-compose.yml   # Orchestration de l'ensemble + MongoDB + RabbitMQ
```
...
Navigateur ──▶ bookstore-web-app (8082) ──▶ gateway (9001) ──▶ inventory (3000) ─┐
                                                             ...
                                                             └─▶ payment (3002)◀┘─┘
                                                                    ▲
                                                     order_created  │ (RabbitMQ, port 5672)
                                                     ────────────────
```
...
### Traitement asynchrone du paiement (RabbitMQ)

A la création d'une commande, `order` publie un message sur la queue durable
`order_created` (RabbitMQ) au lieu d'appeler `payment` en HTTP synchrone.
`payment` consomme ce message et crée automatiquement un paiement au statut
`awaiting-payment`. Ce découplage évite qu'une commande échoue si le service
de paiement est temporairement indisponible, et permet de scaler les deux
services indépendamment. Les deux services retentent la connexion à
RabbitMQ en cas d'indisponibilité au démarrage (cold start du container).

Interface d'administration RabbitMQ : http://localhost:15672 (guest/guest).
...
Cela construit et démarre les 7 containers : `mongo`, `rabbitmq`, `inventory`,
`order`, `payment`, `gateway`, `bookstore-web-app`.
...
cd lb4-order     && npm install && npm run build && MONGO_URL="mongodb://localhost:27017/orderdb" RABBITMQ_URL="amqp://localhost:5672" PORT=3001 npm start
cd lb4-payment   && npm install && npm run build && MONGO_URL="mongodb://localhost:27017/paymentdb" RABBITMQ_URL="amqp://localhost:5672" PORT=3002 npm start
